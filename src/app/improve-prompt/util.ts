...
- 役割を追加する(例: あなたは幼児教育の専門家です。)
- 目標を追加する(例: 幼児の脳の発達をより高めるためのプログラムを作成してください。)
- ユーザー対象を追加する(例: 育児初心者向けに教えてください。)
- 制限を追加する(例: 30分以内で自宅で出来る育児プログラムを作成してください。)
- 出力形式を追加する(例: 結果は表にまとめてください。5つ箇条書きで挙げてください。)
- 「しないこと」を指定するのではなく「すること」を追加する(例: 危ないのは禁止です。→幼児とって安全なプログラムを作成してください。)

# 出力フォーマット

- 改善されたプロンプト
- タスク実行に必要なステップ ステップにはナンバリングはつけないでください。
